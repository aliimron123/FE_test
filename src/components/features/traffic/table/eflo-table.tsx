...
const EfloTabel = ({
  ...
  uniqueGolongan.forEach((gol) => {
    totalPerGolongan[gol] = data
      ...
      .reduce((sum, item) => sum + item.eFlo, 0);
  });
            ...
            <tbody>
              {paginatedData.map((item, index) => (
                <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="border-gray-200 px-4 py-3 text-sm">E-Flo</td>

                  {uniqueGolongan.map((gol, i) => {
                    // hitung total E-Toll per row jika golongan sesuai
                    const totalE = item.Golongan === gol ? item.eFlo : 0;
