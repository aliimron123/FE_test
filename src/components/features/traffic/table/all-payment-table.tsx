...
const CardIdentityTable = ({
  ...
  uniqueGolongan.forEach((gol) => {
    totalPerGolongan[gol] = data
      ...
      .reduce((sum, item) => sum + paymentMethods.reduce((s, k) => s + (item[k] ?? 0), 0), 0);
  });
          ...
          <table className="h-full w-full">
            ...
            <tbody>
              {paginatedData.map((item, index) => {
                // total semua metode pembayaran untuk row ini
                const totalPer = paymentMethods.reduce((sum, key) => sum + (item[key] ?? 0), 0);
                ...
                return (
                  <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="border-gray-200 px-4 py-3 text-sm">All Payments</td>

                    {uniqueGolongan.map((gol, i) => {
                      // hanya tampilkan total golongan ini
                      const totalGol =
                        item.Golongan === gol
                          ? paymentMethods.reduce((sum, key) => sum + (item[key] ?? 0), 0)
                          : 0;
                      return (
                        <td key={i} className="border-gray-200 px-4 py-3 text-center text-sm">
                          {totalGol}
                        </td>
                      );
                    })}

                    {/* Total semua golongan / metode pembayaran */}
                    <td className="px-4 py-3 text-center text-sm font-medium">{totalPer}</td>
                  </tr>
                );
              })}
...
export default CardIdentityTable;
