...
const TrafficTable = ({
  ...
  uniqueGolongan.forEach((gol) => {
    totalPerGolongan[gol] = data
      .filter((item: any) => item.Golongan === gol)
      .reduce(
        (sum, item) => sum + paymentMethods.reduce((s, k) => s + ((item as any)[k] ?? 0), 0),
        0
      );
  });
          ...
          <table className="h-full w-full">
            ...
            <tbody>
              {paginatedData.map((item, index) => {
                const totalPer = paymentMethods.reduce(
                  (sum, key) => sum + ((item as any)[key] ?? 0),
                  0
                );
                ...
                return (
                  <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="border-gray-200 px-4 py-3 text-sm">{paymentTitle}</td>

                    {uniqueGolongan.map((gol, i) => {
                      const totalGol =
                        item.Golongan === gol
                          ? paymentMethods.reduce((sum, key) => sum + ((item as any)[key] ?? 0), 0)
                          : 0;
                      return (
